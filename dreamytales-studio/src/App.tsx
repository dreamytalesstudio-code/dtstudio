...
export default function App() {
  ...
  useEffect(() => {
    // Force homepage on initial load of the website
  ...
  const handleSaveContent = async (newContent: StudioContent): Promise<boolean> => {
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (resData.success && resData.content) {
            ...
            // IMPORTANT: the server can return success:true while having only written to its
            // own local disk (which does not survive a redeploy/restart on most hosts). Only
            // treat this as durably saved if it actually confirmed the Firestore write too.
            persistedDurably = resData.persistedToFirestore === true;
            if (!persistedDurably) {
              console.warn(
                "Server saved content locally but could NOT confirm it was persisted to Firestore. " +
                "Falling back to a direct client-side Firestore write so the change isn't lost on redeploy."
              );
            }
          }
        }
      ...
      if (!persistedDurably) {
        // Save to Firebase directly (ideal for Netlify!) -- this is the durable fallback
        // whenever the server either couldn't be reached or couldn't confirm a Firestore write.
    ...
    <AnimatePresence mode="wait">
      {currentPage === "gallery" ? (
        ...
        <motion.div
          ...
          exit={{ opacity: 0, y: -8 }}
          transition={{ duration: 0.3, ease: [0.25, 1, 0.5, 1] }}
          className="bg-luxury-ivory min-h-screen relative text-luxury-black antialiased"
        >
          ...
          <main className="relative overflow-visible bg-luxury-ivory pb-[4vh]">
            {/* Section 1: Home (Hero) */}
            <div id="home" className="w-full h-screen relative z-10">
            ...
            <CardStackSection id="contact" zIndex={70}>
              ...
              <footer className="bg-luxury-emerald text-luxury-ivory pt-24 pb-16 relative overflow-hidden border-t border-luxury-gold/20">
                <div className="max-w-4xl mx-auto px-6 relative z-10 flex flex-col items-center">
                  ...
                  <div className="relative flex items-center justify-center mb-16 select-none">
                    <div className="relative flex items-center">
                      ...
                      <div className="absolute -left-12 sm:-left-14 top-1/2 -translate-y-1/2 w-20 h-20 sm:w-24 sm:h-24 text-luxury-ivory/80 pointer-events-none z-10">
                        <svg viewBox="0 0 120 120" className="w-full h-full fill-current drop-shadow-[0_4px_12px_rgba(0,0,0,0.3)]">
                      ...
                      <span className="font-serif text-[44px] sm:text-[56px] tracking-[0.2em] text-luxury-ivory leading-none font-light uppercase pl-6 sm:pl-8">
                  ...
                  <div className="grid grid-cols-2 gap-x-16 gap-y-6 sm:gap-x-32 sm:gap-y-8 mb-24 max-w-lg w-full text-center">
                    ...
                    <div className="flex flex-col space-y-5 text-right">
                      <a href="#home" className="font-sans text-[11px] sm:text-xs tracking-[0.35em] font-normal uppercase text-luxury-ivory/80 hover:text-luxury-gold hover:tracking-[0.4em] transition-all duration-300">
                        HOME
                      </a>
                      <a href="#about-us" className="font-sans text-[11px] sm:text-xs tracking-[0.35em] font-normal uppercase text-luxury-ivory/80 hover:text-luxury-gold hover:tracking-[0.4em] transition-all duration-300">
                        ABOUT
                      </a>
                      <button
                        onClick={() => navigateTo("gallery")}
                        className="font-sans text-[11px] sm:text-xs tracking-[0.35em] font-normal uppercase text-luxury-ivory/80 hover:text-luxury-gold hover:tracking-[0.4em] transition-all duration-300 text-right bg-transparent border-none cursor-pointer focus:outline-none"
                      >
                        GALLERY
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col space-y-5 text-left">
                      <a href="#experience" className="font-sans text-[11px] sm:text-xs tracking-[0.35em] font-normal uppercase text-luxury-ivory/80 hover:text-luxury-gold hover:tracking-[0.4em] transition-all duration-300">
                        LEGACY
                      </a>
                      <a href="#praise" className="font-sans text-[11px] sm:text-xs tracking-[0.35em] font-normal uppercase text-luxury-ivory/80 hover:text-luxury-gold hover:tracking-[0.4em] transition-all duration-300">
                        REVIEWS
                      </a>
                      <a href="#contact" className="font-sans text-[11px] sm:text-xs tracking-[0.35em] font-normal uppercase text-luxury-ivory/80 hover:text-luxury-gold hover:tracking-[0.4em] transition-all duration-300">
                        INQUIRE
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="w-full border-t border-luxury-gold/10 pt-10 text-center text-luxury-ivory/60 font-sans tracking-[0.18em] text-[8px] sm:text-[9px] uppercase leading-relaxed font-light space-y-4">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 max-w-2xl mx-auto">
                      ...
                      <button
                        ...
                        className="hover:text-luxury-gold transition-colors cursor-pointer font-sans text-[8px] sm:text-[9px] uppercase tracking-[0.18em] focus:outline-none text-luxury-ivory/60 bg-transparent"
                      >
                        T&amp;C and PRIVACY POLICY
                      </button>
                    </div>
                    <div className="pt-2 text-[8px] tracking-[0.25em] text-luxury-gold/60">
