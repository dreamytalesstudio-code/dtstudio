...
export default function GalleryPage({ portfolioItems = [], brandName, onBack }: GalleryPageProps) {
  ...
  useEffect(() => {
    if (allPhotos && allPhotos.length > 0) {
      console.log("Display side GalleryPage rendering photo/video URLs:");
      allPhotos.forEach(photo => {
        if (photo.mediaUrl) {
          console.log(`- Gallery Item [${photo.title}]: Rendering URL -> ${photo.mediaUrl}`);
        }
      });
    }
  }, [allPhotos]);
  ...
  useEffect(() => {
    ...
    offsets.forEach((offset) => {
      ...
      if (item && item.mediaUrl && item.mediaType === "image") {
        urlsToPreload.push(item.mediaUrl);
      }
    });
  ...
  return (
    <div className="min-h-screen bg-luxury-ivory text-luxury-espresso antialiased flex flex-col">
      {/* Dynamic Luxurious Header */}
      <header className="sticky top-0 z-40 bg-luxury-ivory/95 backdrop-blur-md border-b border-luxury-gold/15 px-6 py-5 select-none">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <button
            onClick={onBack}
            className="flex items-center gap-2.5 text-xs tracking-[0.25em] uppercase font-sans text-luxury-espresso hover:text-luxury-gold transition-colors group cursor-pointer font-medium"
          >
            <ArrowLeft className="w-4 h-4 transition-transform duration-300 group-hover:-translate-x-1" />
            <span>Home</span>
          </button>

          <span className="font-serif text-lg sm:text-2xl tracking-[0.25em] font-light uppercase text-luxury-espresso">
            {brandName || "DTSTUDIO"}
          </span>

          <div className="flex items-center gap-2 text-xs tracking-[0.2em] uppercase font-sans text-luxury-charcoal/70">
            <Grid className="w-3.5 h-3.5 text-luxury-espresso" />
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 md:px-12 py-12 md:py-16 w-full">
        ...
        <div className="text-center mb-16">
          <span className="font-sans text-[10px] md:text-xs uppercase tracking-[0.6em] text-luxury-blush font-semibold block mb-3">
            IMMERSIVE SHOWCASE
          </span>
          <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl text-luxury-espresso font-light tracking-wide leading-tight">
            Complete Archive
          </h1>
          <div className="w-16 h-[1.5px] bg-gradient-to-r from-luxury-blush via-luxury-gold to-luxury-coral mx-auto mt-6 relative overflow-hidden">
            <div className="absolute inset-0 bg-white/40 translate-x-[-100%] animate-[shimmer_3s_infinite]" />
          </div>
        </div>

        {/* Categories Tab Bar */}
        <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 md:gap-x-10 mb-16 border-b border-luxury-gold/15 pb-6">
            ...
            <button
              ...
              className={`font-sans text-[10px] sm:text-xs tracking-[0.25em] uppercase pb-2 transition-all duration-300 relative cursor-pointer font-medium ${
                activeFilter === category
                  ? "text-luxury-espresso font-semibold"
                  : "text-luxury-charcoal/60 hover:text-luxury-espresso"
              }`}
            >
              {category}
              {activeFilter === category && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-0 right-0 h-[2.5px] bg-gradient-to-r from-luxury-blush via-luxury-gold to-luxury-coral"
            ...
            <AnimatePresence mode="popLayout">
              {filteredPhotos.map((item, index) => (
                <motion.div
                  ...
                  className="group relative cursor-pointer flex flex-col bg-luxury-ivory overflow-hidden border border-luxury-gold/15"
                >
                  {/* Photo Frame */}
                  <div className="relative overflow-hidden aspect-[3/4] bg-luxury-ivory shadow-sm">
                    {item.mediaType === "video" ? (
                      <div className="w-full h-full relative">
                        <LazyMedia
                          ...
                          className="contrast-[1.03] brightness-[1.01] transition-transform duration-700 group-hover:scale-105"
                        />
                        <div className="absolute inset-0 flex items-center justify-center bg-black/10 z-10">
                          <div className="w-12 h-12 rounded-full border border-luxury-ivory/20 bg-luxury-ivory/10 backdrop-blur-md flex items-center justify-center text-luxury-ivory scale-90 group-hover:scale-100 group-hover:bg-luxury-gold group-hover:text-luxury-ivory transition-all duration-300">
                  ...
                  <div className="pt-4 pb-2 px-1 text-center sm:text-left">
                    <span className="text-[9px] uppercase tracking-[0.25em] text-luxury-blush block mb-1 font-sans font-medium">
                      {item.category}
                    </span>
                    <h3 className="font-serif text-[11px] sm:text-xs tracking-[0.15em] text-luxury-espresso uppercase font-medium">
                      {item.title}
                    </h3>
                  </div>
                </motion.div>
          ...
          >
            ...
            <div className="absolute top-6 left-6 right-6 flex items-center justify-between text-white/80 z-50 cursor-default" onClick={(e) => e.stopPropagation()}>
              <div className="flex flex-col text-left">
                <span className="font-sans text-[10px] uppercase tracking-[0.3em] text-luxury-blush">
              ...
              <div className="flex items-center space-x-3">
                {currentItem.mediaType === "video" && (
                   <button
                     ...
                     className="p-2.5 rounded-full bg-zinc-900/80 hover:bg-luxury-gold text-luxury-ivory transition-all border border-luxury-gold/30 cursor-pointer"
                     title="Toggle Audio"
                   >
                ...
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 sm:px-5 sm:py-2.5 rounded-full bg-luxury-ivory text-luxury-espresso hover:bg-luxury-gold hover:text-luxury-ivory border border-luxury-gold transition-all cursor-pointer shadow-xl"
              ...
              <div className="max-w-full max-h-full rounded overflow-hidden shadow-2xl flex items-center justify-center bg-black border border-white/5" onClick={(e) => e.stopPropagation()}>
                {currentItem.mediaType === "video" ? (
                  ...
                  <img
                    ...
                    className={`max-w-full max-h-[65vh] md:max-h-[78vh] object-contain transition-opacity duration-500 ease-out ${
            ...
            {/* Bottom Carousel Indicator removed */}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
