    ...
    <header
      ...
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        ...
        isScrolled || currentPath !== "home"
          ? "bg-luxury-ivory/90 backdrop-blur-xl border-b border-luxury-gold/20 py-3.5 shadow-[0_8px_32px_rgba(43,36,32,0.03)]"
          : "bg-luxury-espresso/15 backdrop-blur-md border-b border-luxury-ivory/15 py-5"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        <a href="#home" onClick={(e) => handleLinkClick(e, "#home")} className="flex items-center space-x-3 group">
          <div className={`w-10 h-10 border flex items-center justify-center rounded-full transition-colors duration-500 ${
            isScrolled || currentPath !== "home" ? "border-luxury-espresso/60 group-hover:border-luxury-espresso" : "border-luxury-ivory/40 group-hover:border-luxury-ivory"
          }`}>
            <span className={`font-serif text-sm tracking-widest transition-colors ${
              isScrolled || currentPath !== "home" ? "text-luxury-espresso group-hover:text-luxury-blush" : "text-luxury-ivory group-hover:text-luxury-blush"
            }`}>DT</span>
          </div>
          <div className="flex flex-col">
            <span className={`font-serif text-base md:text-lg tracking-widest transition-colors duration-300 font-medium ${
              isScrolled || currentPath !== "home" ? "text-luxury-espresso group-hover:text-luxury-espresso" : "text-luxury-ivory group-hover:text-luxury-ivory"
            }`}>
              DREAMY TALES
            </span>
            <span className={`font-sans text-[8px] md:text-[9px] tracking-[0.25em] -mt-1 uppercase font-semibold ${
              isScrolled || currentPath !== "home" ? "text-luxury-charcoal/80" : "text-luxury-ivory/60"
            }`}>
              Studio & Films
            </span>
          </div>
        ...
        <nav className="hidden lg:flex items-center space-x-8">
          {navLinks.map((link) => {
            ...
            return (
              <a
                ...
                className={`font-sans text-xs uppercase tracking-[0.2em] transition-all duration-300 relative py-1 nav-link-dreamy ${
                  isActive
                    ? "text-luxury-gold font-bold active-nav-link"
                    : isScrolled || currentPath !== "home"
                      ? "text-luxury-charcoal hover:text-luxury-gold"
                      : "text-luxury-ivory/80 hover:text-luxury-ivory"
                }`}
              >
                {link.name}
              </a>
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3 py-2 border text-[10px] uppercase tracking-[0.15em] transition-all duration-300 cursor-pointer rounded-sm ${
              isScrolled || currentPath !== "home"
                ? "border-luxury-gold/30 text-luxury-charcoal hover:border-luxury-gold hover:text-luxury-gold hover:bg-luxury-gold/5"
                : "border-luxury-ivory/30 text-luxury-ivory/80 hover:border-luxury-ivory hover:text-luxury-ivory hover:bg-luxury-ivory/10"
            }`}
          >
            <span>T&C</span>
          </button>
        ...
        <div className="flex lg:hidden items-center space-x-3">
          <button
            ...
            className={`px-2 py-1.5 rounded-sm border text-[9px] font-bold uppercase tracking-[0.12em] ${
              isScrolled || currentPath !== "home"
                ? "border-luxury-gold/30 text-luxury-charcoal hover:text-luxury-gold"
                : "border-luxury-ivory/20 text-luxury-ivory/80 hover:text-luxury-ivory"
            }`}
          >
            T&C
          </button>
          ...
          <button
            ...
            className={`p-2 transition-colors ${
              isScrolled || currentPath !== "home" ? "text-luxury-espresso hover:text-luxury-gold" : "text-luxury-ivory hover:text-luxury-gold"
            }`}
          >
      ...
      {isMobileMenuOpen && (
        <div className={`lg:hidden absolute top-full left-0 w-full backdrop-blur-xl border-b py-6 px-8 flex flex-col space-y-6 shadow-[0_12px_40px_rgba(43,36,32,0.08)] animate-fade-in ${
          isScrolled || currentPath !== "home"
            ? "bg-luxury-ivory/95 border-luxury-gold/15" 
            : "bg-luxury-espresso/95 border-luxury-ivory/10"
        }`}>
          {navLinks.map((link) => {
            const isActive = currentPath === link.href.replace("#", "") || (link.href === "#home" && currentPath === "home");
            return (
              <a
                ...
                className={`font-sans text-sm uppercase tracking-[0.2em] py-1 border-b transition-colors ${
                  isActive
                    ? "text-luxury-gold font-semibold border-luxury-gold"
                    : isScrolled || currentPath !== "home"
                      ? "text-luxury-charcoal hover:text-luxury-gold border-luxury-gold/5" 
                      : "text-luxury-ivory/80 hover:text-luxury-ivory border-luxury-ivory/5"
                }`}
              >
                {link.name}
              </a>
