...
import { Play, X, Volume2, VolumeX, ChevronLeft, ChevronRight, Loader2, Sparkles } from "lucide-react";
...
export default function Portfolio({ items = [], onViewMore }: PortfolioProps) {
  ...
  useEffect(() => {
    if (allPortfolioItems && allPortfolioItems.length > 0) {
      console.log("Display side Portfolio rendering image/video URLs:");
      allPortfolioItems.forEach(item => {
        if (item.mediaUrl) {
          console.log(`- Portfolio Item [${item.title}]: Rendering URL -> ${item.mediaUrl}`);
        }
      });
    }
  }, [allPortfolioItems]);
  ...
  return (
    <section id="portfolio" className="relative py-14 md:py-20 bg-luxury-ivory">
      <div className="max-w-5xl mx-auto px-6 md:px-12">
        
        {/* Elegant Luxury Section Header */}
        <div className="mb-10 text-center">
          <span className="font-sans text-[10.5px] md:text-xs uppercase tracking-[0.5em] text-luxury-espresso mb-3 block font-semibold flex items-center justify-center gap-2">
            <Sparkles className="w-3.5 h-3.5 text-luxury-gold animate-dreamy-sparkle" />
            VISUAL CATALOGUE
            <Sparkles className="w-3.5 h-3.5 text-luxury-gold animate-dreamy-sparkle" />
          </span>
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl text-luxury-espresso tracking-wide font-light leading-tight">
            Films &amp; Photos
          </h2>
        ...
        {/* Categories Tab Bar */}
        <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 md:gap-x-10 mb-12 border-b border-luxury-gold/15 pb-6">
        ...
        <motion.div
          layout
          className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-12 md:gap-x-14 md:gap-y-16 lg:gap-x-16 lg:gap-y-20 max-w-5xl mx-auto"
        >
              ...
              return (
                <motion.div
                  ...
                  className={`group relative cursor-pointer flex flex-col transition-all duration-500 ease-out hover:-translate-y-2.5 ${
                    isEvenColumn ? "md:mt-12" : ""
                  }`}
                >
                  {/* Media Frame with Sharp Rectangular Edges */}
                  <div className="relative overflow-hidden aspect-[4/5] bg-luxury-ivory shadow-[0_12px_40px_rgba(43,36,32,0.03)] hover:shadow-[0_24px_50px_rgba(43,36,32,0.08)] border border-luxury-gold/15 transition-all duration-500 ease-out">
                    {item.mediaType === "video" ? (
                      <div className="w-full h-full relative">
                        <LazyMedia
                          ...
                          className="contrast-[1.05] brightness-[1.01] transition-all duration-1000 group-hover:scale-[1.03] group-hover:contrast-[1.1]"
                        />
                        {/* Elegant Glassmorphic Play button */}
                        <div className="absolute inset-0 flex items-center justify-center bg-black/5 z-10">
                          <div className="w-14 h-14 rounded-full border border-luxury-ivory/30 bg-luxury-ivory/20 backdrop-blur-md flex items-center justify-center text-luxury-ivory scale-95 group-hover:scale-100 group-hover:bg-luxury-gold group-hover:text-luxury-ivory transition-all duration-500">
                  ...
                  <div className="text-center mt-4">
                    <span className="text-[9px] uppercase tracking-[0.25em] text-luxury-blush block mb-1 font-sans">
                      {item.category}
                    </span>
        ...
        <div className="mt-16 text-center select-none">
          <button
            ...
            className="px-10 py-4 btn-dreamy btn-dreamy-pulse text-xs uppercase tracking-[0.2em] font-bold rounded-sm hover:scale-[1.03] active:scale-[0.98] cursor-pointer inline-flex items-center justify-center gap-2"
          >
            {isLoadingMore ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin text-luxury-espresso" />
                <span>Loading...</span>
              </>
            ) : onViewMore ? (
              ...
              <div className="max-w-full max-h-full rounded-lg overflow-hidden shadow-2xl flex items-center justify-center bg-luxury-black border border-white/5" onClick={(e) => e.stopPropagation()}>
                {currentItem.mediaType === "video" ? (
                  ...
                  <img
                    ...
                    className={`max-w-full max-h-[60vh] md:max-h-[75vh] object-contain transition-opacity duration-500 ease-out ${
