...
// Writes locally (fast, always succeeds) AND awaits the Firestore write so callers
// know for certain whether the change is durably persisted. content.json alone lives
// on local/ephemeral disk and will NOT survive a redeploy or container restart on most
// hosts -- Firestore is the durable copy. Previously this Firestore write was
// fire-and-forget, so failures (e.g. missing/invalid FIREBASE_SERVICE_ACCOUNT_KEY) were
// only ever logged server-side and silently discarded, causing saved changes to
// disappear on the next redeploy with no visible error anywhere.
const writeContent = async (content: StudioContent): Promise<{ persistedToFirestore: boolean; firestoreError?: string }> => {
  fs.writeFileSync(CONTENT_FILE, JSON.stringify(content, null, 2));
  if (firestoreDb) {
    try {
      await firestoreDb.collection("settings").doc("content").set(content);
      console.log("Saved content to Firestore settings/content successfully.");
      return { persistedToFirestore: true };
    } catch (err: any) {
      console.error("Failed to save content to Firestore settings/content:", err);
      return { persistedToFirestore: false, firestoreError: err.message || String(err) };
    }
  }
  return { persistedToFirestore: false, firestoreError: "Firestore is not configured on this server." };
};
...
const writeInquiries = async (inquiries: Inquiry[]): Promise<{ persistedToFirestore: boolean; firestoreError?: string }> => {
  fs.writeFileSync(INQUIRIES_FILE, JSON.stringify(inquiries, null, 2));
  if (firestoreDb) {
    try {
      await firestoreDb.collection("settings").doc("inquiries").set({ list: inquiries });
      console.log("Saved inquiries to Firestore settings/inquiries successfully.");
      return { persistedToFirestore: true };
    } catch (err: any) {
      console.error("Failed to save inquiries to Firestore settings/inquiries:", err);
      return { persistedToFirestore: false, firestoreError: err.message || String(err) };
    }
  }
  return { persistedToFirestore: false, firestoreError: "Firestore is not configured on this server." };
...
app.post("/api/content", async (req, res) => {
  try {
    ...
    const { persistedToFirestore, firestoreError } = await writeContent(mergedContent);
    if (!persistedToFirestore) {
      // Local content.json was written and this request will look/feel successful, but
      // on most hosts that local file does NOT survive a redeploy or restart -- Firestore
      // is the only durable copy. Surface this loudly so it doesn't look like data loss
      // "just happens" later with no explanation.
      console.error(
        `WARNING: Content saved to local disk only, NOT to Firestore (${firestoreError}). ` +
        `This change WILL be lost on the next server restart/redeploy unless Firestore is configured correctly.`
      );
    }
    res.json({
      success: true,
      message: persistedToFirestore
        ? "Content updated successfully!"
        : "Content saved, but could NOT be persisted to Firestore -- it will likely be lost on the next restart/redeploy. Check server logs / Firestore configuration.",
...
app.post("/api/inquiries", async (req, res) => {
  try {
    ...
    const { persistedToFirestore, firestoreError } = await writeInquiries(inquiries);
    if (!persistedToFirestore) {
      console.error(`WARNING: Inquiry saved to local disk only, NOT to Firestore (${firestoreError}).`);
    }

    res.json({ success: true, message: "Inquiry submitted successfully! We will get back to you shortly.", inquiry: newInquiry, persistedToFirestore });
  } catch (err: any) {
...
app.post("/api/inquiries/:id/status", async (req, res) => {
  try {
    ...
    const { persistedToFirestore } = await writeInquiries(inquiries);

    res.json({ success: true, inquiries, persistedToFirestore });
  } catch (err: any) {
    res.status(500).json({ error: err.message || "Failed to update inquiry status." });
  }
});

// Verify Firestore is actually reachable and writable, not just "initialized".
// getFirestore() succeeds even with missing/invalid credentials -- it only throws once you
// try to actually read/write. Without this check, a broken Firestore connection is
// completely silent: saves "succeed" locally, then vanish on the next restart/redeploy
// with no error visible anywhere.
async function verifyFirestoreConnection() {
  if (!firestoreDb) {
    console.warn(
      "!!! FIRESTORE NOT CONFIGURED !!! content.json/inquiries.json will only live on local disk. " +
      "On most hosting platforms local disk does NOT survive a redeploy or restart -- any changes " +
      "made in the admin panel (including uploaded portfolio images) WILL be lost. " +
      "Set FIREBASE_SERVICE_ACCOUNT_KEY (or valid Application Default Credentials) to fix this."
    );
    return;
  }
  try {
    const healthRef = firestoreDb.collection("settings").doc("_health_check");
    await healthRef.set({ checkedAt: new Date().toISOString() });
    await healthRef.get();
    console.log("Firestore connection verified: read/write OK. Saved content will survive restarts/redeploys.");
  } catch (err: any) {
    console.error(
      "!!! FIRESTORE CONNECTION FAILED !!! Firestore was initialized but a real read/write failed: " +
      `${err.message || err}. ` +
      "content.json/inquiries.json will only live on local disk and WILL be lost on the next " +
      "restart/redeploy. Check FIREBASE_SERVICE_ACCOUNT_KEY / service account permissions."
    );
  }
}
...
async function startServer() {
  // Verify Firestore is actually usable before relying on it
